...
def get_summary(db: Session) -> SummaryResponse:
    ...
    total_income = db.query(func.sum(Record.amount))\
                    .filter(Record.type == RecordType.income)\
                    .scalar() or 0.0
    
    # Sum expense
    total_expense = db.query(func.sum(Record.amount))\
                    .filter(Record.type == RecordType.expense)\
                    .scalar() or 0.0
...
def get_category_breakdown(db: Session) -> Dict[str, float]:
    ...
    # If the user wants total across both types, we just sum it.
    # Usually, a breakdown might separate income/expense, 
    # but the prompt asks for category -> total_amount.
    results = db.query(Record.category, func.sum(Record.amount))\
